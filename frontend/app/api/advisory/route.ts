...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(DEMO_ADVISORIES[key] || DEMO_ADVISORIES["solan-tomato"]);
  } catch {
    return NextResponse.json(DEMO_ADVISORIES["solan-tomato"]);
  }
}

export async function GET() {
  return NextResponse.json({
    service: "KisanMind Advisory API", version: "1.0.0",
    agents: ["SatDrishti", "MandiMitra", "MausamGuru", "VaaniSetu"], status: "running",
  });
}
